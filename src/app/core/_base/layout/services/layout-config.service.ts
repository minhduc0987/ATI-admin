...
export class LayoutConfigService {
	...
	getConfig(path?: string): LayoutConfigModel | any {
		...
		// @todo; known issue; viewing 2 or more projects at the time in different browser's tabs, can cause conflict to the layout config
	...
	loadConfigs(config: LayoutConfigModel) {
		this.layoutConfig = this.getSavedConfig();
		// use saved config as priority, or load new config if project does not matched
		if (!this.layoutConfig || objectPath.get(this.layoutConfig, 'project') !== config.project) {
