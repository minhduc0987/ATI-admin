...
import {
	...
	selectCampaignUpload,
	selectCampaignUploadError,
} from '../../../../../core/apps/_selectors/file-upload.selectors';
import { HttpClient, HttpErrorResponse } from '@angular/common/http';
import { Subscription, Observable } from 'rxjs';

@Component({
	selector: 'kt-image-preview-campaign',
	templateUrl: './image-preview-campaign.component.html',
	styleUrls: ['./image-preview-campaign.component.scss'],
	encapsulation: ViewEncapsulation.None,
})
export class ImagePreviewCampaignComponent implements OnInit, OnDestroy {
	 ...
	 */
	constructor(
		public dialogRef: MatDialogRef<ImagePreviewCampaignComponent>,
	...
	uploadFile(event) {
		...
		this.store.dispatch(new CampaignUploadMutil({ documentType: this.data.documentType, files: formData }));
		this.viewLoading = true;
		event = null;
		if (!this.isUploadCall) {
			this.isUploadCall = true;
			this.subcriptions.add(
				this.store.pipe(select(selectCampaignUpload)).subscribe((res) => {
					if (res) {
						res.files.map(x => {
							this.addFormArray(x);
						});
			...
			this.subcriptions.add(
				this.store.pipe(select(selectCampaignUploadError)).subscribe(err => {
					if (err) {
