...
import { UserProfileService } from '../../../core/apps';
...
export class AsideLeftComponent implements OnInit, AfterViewInit {
	...
	getUserServiceCode() {
		...
		if (currentUser) {
			...
			if (role) {
				role.forEach(fnRole => {
					if (fnRole.functions) {
						fnRole.functions.forEach(e => {
							if (!svCode.includes(e.services[0].serviceCode)) {
								svCode.push(e.services[0].serviceCode);
							}
						});
					}
				});
	...
	isSubmenuHidden(element): boolean {
		let isHidden = false;
		const userSvCode = this.getUserServiceCode();
		if (element.serviceCode) {
			userSvCode.forEach(e => {
				if (element.serviceCode === e) {
					isHidden = true;
				}
			});
		} else {
	...
	// tslint:disable-next-line: no-shadowed-variable
	isMenuHidden(element): boolean {
		const userSvCode = this.getUserServiceCode();
		let isHidden = 0;
		if (element.submenu) {
			element.submenu.forEach(e => {
				if (e.serviceCode) {
					userSvCode.forEach(svCode => {
						if (e.serviceCode === svCode) {
							isHidden = 1;
						}
					});
				} else {
	...
	getClassesSubmenu(item) {
		let classes = 'kt-menu__item';

		if (this.isSubmenuHidden(item)) {
			classes += ' dis-block';
		}
		if (!this.isSubmenuHidden(item)) {
			classes += ' dis-none';
